...
const CLIENTS_KEY = "gladiama-clients";
const INVOICES_KEY = "gladiama-invoices";
...
export function getStoredClients(): ClientRecord[] {
  ...
  try {
    ...
    if (!saved) {
      window.localStorage.setItem(CLIENTS_KEY, JSON.stringify(defaultClients));
...
export function updateClient(clientId: string, nextData: Partial<ClientRecord>) {
  const clients = getStoredClients();
  const updated = clients.map((client) => (client.id === clientId ? { ...client, ...nextData } : client));
  saveClients(updated);
  return updated;
}

export function deleteClient(clientId: string) {
  const clients = getStoredClients().filter((client) => client.id !== clientId);
  saveClients(clients);
  return clients;
}
...
export function getStoredInvoices(): InvoiceRecord[] {
  ...
  try {
    ...
    if (!saved) {
      window.localStorage.setItem(INVOICES_KEY, JSON.stringify(defaultInvoices));
      return defaultInvoices;
    }
    const parsed = JSON.parse(saved);
    return Array.isArray(parsed) && parsed.length ? parsed : defaultInvoices;
  } catch {
    return defaultInvoices;
  }
}
...
export function getNextInvoiceNumber(existing: InvoiceRecord[] = getStoredInvoices()): string {
  const highest = existing.reduce((max, invoice) => {
    const parsed = Number.parseInt(invoice.number.replace(/\D+/g, ""), 10);
...
export function buildInvoiceRecord(input: {
  ...
  return {
    ...
    date: new Intl.DateTimeFormat("fr-FR", { day: "2-digit", month: "long", year: "numeric" }).format(new Date(createdAt)).replace(/(^|\s)([a-z])/g, (_, prefix: string, letter: string) => `${prefix}${letter.toUpperCase()}`),
...
export function updateInvoiceStatus(invoiceNumber: string, status: InvoiceStatus) {
  ...
  const updated = invoices.map((invoice) => (invoice.number === invoiceNumber ? { ...invoice, status } : invoice));
  saveInvoices(updated);
  return updated;
}

export function deleteInvoice(invoiceNumber: string) {
  const invoices = getStoredInvoices().filter((invoice) => invoice.number !== invoiceNumber);
